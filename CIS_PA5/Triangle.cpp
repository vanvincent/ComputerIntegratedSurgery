...
Triangle::Triangle(Vector3d a,Vector3d b,Vector3d c,int index){
	id = index;
	vertex[0] = a;
	vertex[1] = b;
	vertex[2] = c;
...
Vector3d Triangle::barycentric(const Vector3d target){
	Vector3d coefficient;
	//barycentric coordinate coefficients
	double area1 = (((vertex[2]-vertex[0]).cross(vertex[1]-vertex[0]))).norm();
	double area2 = (((vertex[2]-target).cross(vertex[1]-target))).norm();
	double area3 = (((vertex[0]-target).cross(vertex[1]-target))).norm();
	coefficient(0) = area2 / area1;
	coefficient(2) = area3 / area1;
	coefficient(1) = 1 - coefficient(0) -coefficient(2);
	return coefficient;
}
